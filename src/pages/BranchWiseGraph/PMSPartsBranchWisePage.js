...
function PMSPartsBranchWisePage() {
  ...
  useEffect(() => {
    const saved = getSelectedGrowth("pms_parts");
    ...
    const fetchCitySummary = async () => {
      try {
        ...
        for (const m of activeMonths) {
          ...
          const data = await fetchData(`/api/pms_parts/pms_parts_branch_summary${query}`);
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h4">PMS PARTS GRAPH (BranchWise)</Typography>
...
export default PMSPartsBranchWisePage;
