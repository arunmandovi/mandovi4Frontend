...
function BatteryTyreBranchWisePage() {
    ...
    const fetchCitySummary = async () => {
      try {
        ...
        for (const m of activeMonths) {
          ...
          const data = await fetchData(`/api/battery_tyre/battery_tyre_branch_summary${query}`);
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h4">BATTERY & TYRE GRAPH (BranchWise)</Typography>
...
export default BatteryTyreBranchWisePage;
