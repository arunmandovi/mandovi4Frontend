    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end", mb: 3 }}>
        <FormControl size="small" sx={{ minWidth: 260 }}>
          ...
          <Select multiple value={selectedBranches} label="Select Branches" onChange={handleBranchChange} renderValue={(selected) => selected.join(", ")}>

            <ListItemText primary="Bangalore" sx={{ pl: 2, fontWeight: "bold" }} />
            {Object.entries(BRANCH_CITY_MAP).filter(([_, city]) => city === "Bangalore").map(([branch]) => (
            ...
            {Object.entries(BRANCH_CITY_MAP).filter(([_, city]) => city === "Mysore").map(([branch]) => (
              <MenuItem key={branch} value={branch}>
                <Checkbox checked={selectedBranches.includes(branch)} />
                <ListItemText primary={branch} />
              </MenuItem>
            ...
            {Object.entries(BRANCH_CITY_MAP).filter(([_, city]) => city === "Mangalore").map(([branch]) => (
              <MenuItem key={branch} value={branch}>
                <Checkbox checked={selectedBranches.includes(branch)} />
                <ListItemText primary={branch} />
              </MenuItem>
