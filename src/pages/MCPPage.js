...
function MCPPage() {
  const mcpConfig = apiModules.find((m) => m.name === "MCP");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = mcpConfig.get;
      if (withFilter && month && year) {
        path = mcpConfig.getMCPByMonthYear
      ...
      alert("❌ Error fetching MCP: " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(mcpConfig.upload, file);
      alert("✅ MCP file uploaded successfully!");
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Table */}
      <DataTable data={tableData} title="MCP" />
    </Box>
  );
}
