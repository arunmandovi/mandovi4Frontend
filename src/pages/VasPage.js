...
function VasPage() {
  const vasConfig = apiModules.find((m) => m.name === "VAS");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = vasConfig.get
      if (withFilter && month && year) {
        path = vasConfig.getVASByMonthYear
      ...
      alert("❌ Error fetching VAS: " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(vasConfig.upload, file);
      alert("✅ VAS file uploaded successfully!");
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Table */}
      <DataTable data={tableData} title="VAS" />
    </Box>
  );
}
