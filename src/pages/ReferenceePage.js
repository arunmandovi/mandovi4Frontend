...
function ReferenceePage() {
  const [referenceeSummary, setReferenceeSummary] = useState([]);
  ...
  const columnRenameMap = {
    ...
    booking: "Booking",
    invoice: "Invoice",
    percentageEnquiryBooking: "E-B",
    percentageEnquiryInvoice: "E-R",
    percentageBookingInvoice: "B-R",
  };
...
const addGrandTotalRow = (data) => {
  ...
  allKeys.forEach((key) => {
    ...
    if (key.toLowerCase().includes("percentage")) {
      ...
      if (key.includes("percentageEnquiryBooking")) {
        const booking = totalRow["booking"] || 0;
        const enquiry = totalRow["enquiry"] || 0;
        sum = booking * 100 / enquiry;
      } else if (key.includes("percentageEnquiryInvoice")) {
        const invoice = totalRow["invoice"] || 0;
        const enquiry = totalRow["enquiry"] || 0;
        sum = invoice * 100 / enquiry;
      } else if (key.includes("percentageBookingInvoice")) {
        const invoice = totalRow["invoice"] || 0;
        const booking = totalRow["booking"] || 0;
        sum = invoice * 100 / booking;
      } else {
        sum = val;
      }
  ...
  useEffect(() => {
    const fetchReferenceeSummary = async () => {
      try {
        let responses = [];
        ...
        setReferenceeSummary(withTotal);
      } catch (error) {
        console.error(error);
    ...
    fetchReferenceeSummary();
  }, [months, quarters, halfYears, groupBy]);

  const renamedData = referenceeSummary.map((row) => {
    const newRow = {};
  ...
  return (
    <Box className="battery-container" sx={{ p: 3 }}>
      ...
      <Box sx={{ overflowX: "auto", width: "100%" }}>
        <DataTable data={renamedData} title="Reference Summary" hiddenColumns={hiddenColumns} />
      </Box>
    </Box>
  );
}
