...
function DueDoneUploadPage() {
  const config = apiModules.find((m) => m.name === "Due Done");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Delete ALL Due VS Done data?")) return;

    setLoading(true);
    try {
      await deleteAllRecords(config.deleteAll);
      alert("🗑 All Due VS Done data deleted!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="Due VS Done Upload File" />
...
export default DueDoneUploadPage;
