...
function BRConversionUploadPage() {
  const brConversionConfig = apiModules.find((m) => m.name === "BR Conversion");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = brConversionConfig.get;
      if (withFilter && month && year) {
        path = brConversionConfig.getBRConversionByMonthYear
      ...
      alert("❌ Error fetching BR Conversion : " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(brConversionConfig.upload, file);
      alert("✅ BR Conversion file uploaded successfully!");
      setFile(null);
    } catch (err) {
  ...
  const columns = tableData[0]
    ? Object.keys(tableData[0])
        .filter((key) => key !== "br_conversionSINo")
        .map((key) => ({
          field: key,
          flex: 1,
        }))
...
export default BRConversionUploadPage;