...
function BRConversionUploadPage() {
  const config = apiModules.find((m) => m.name === "BR Conversion");
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      const url = `${config.getBRConversionByMonthYear}?${params.toString()}`;
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Are you sure you want to delete ALL BR Conversion data? This action cannot be undone.")) {
      return;
    }

    try {
      const res = await deleteData(config.deleteAll);
      alert("🗑️ All BR Conversion data deleted successfully!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="BR Conversion Upload File" />
