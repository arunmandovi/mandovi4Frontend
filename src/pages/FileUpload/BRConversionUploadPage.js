...
function BRConversionUploadPage() {
  ...
  const handleFetch = async () => {
    try {
      const data = await fetchData(brConversionConfig.get);
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      const url = `${brConversionConfig.getBRConversionByMonthYear}?${params.toString()}`;
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
              ...
              ></Box>
        <TitleBar
          title="BR Conversion Upload File"
