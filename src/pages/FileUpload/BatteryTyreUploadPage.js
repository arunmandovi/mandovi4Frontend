...
function BatteryTyreUploadPage() {
  const config = apiModules.find((m) => m.name === "Battery & Tyre");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Are you sure you want to delete ALL Battery & Tyre data? This action cannot be undone.")) {
      return;
    }

    try {
      const res = await deleteData(config.deleteAll);
      alert("🗑️ All Battery & Tyre data deleted successfully!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="Battery & Tyre Upload File" />
