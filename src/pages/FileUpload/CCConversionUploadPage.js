...
function CCConversionUploadPage() {
  const config = apiModules.find((m) => m.name === "CC Conversion");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Delete ALL CC Conversion data?")) return;

    setLoading(true);
    try {
      await deleteAllRecords(config.deleteAll);
      alert("🗑 All CC Conversion data deleted!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="CC Conversion Upload File" />
...
export default CCConversionUploadPage;
