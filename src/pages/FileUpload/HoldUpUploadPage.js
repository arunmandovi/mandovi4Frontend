...
function HoldUpUploadPage() {
  const config = apiModules.find((m) => m.name === "Hold Up");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Delete ALL Hold Up data?")) return;

    setLoading(true);
    try {
      await deleteAllRecords(config.deleteAll);
      alert("🗑 All Hold Up data deleted!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="Hold Up Upload File" />
...
export default HoldUpUploadPage;
