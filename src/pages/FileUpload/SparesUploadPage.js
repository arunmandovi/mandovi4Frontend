...
function SparesUploadPage() {
  const config = apiModules.find((m) => m.name === "Spares");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Are you sure you want to delete ALL Spares data? This action cannot be undone.")) {
      return;
    }

    try {
      const res = await deleteData(config.deleteAll);
      alert("🗑️ All Spares data deleted successfully!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="Spares Upload File" />
