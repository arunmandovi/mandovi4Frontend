...
function SparesUploadPage() {
  const sparesConfig = apiModules.find((m) => m.name === "Spares");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = sparesConfig.get;
      if (withFilter && month && year) {
        path = sparesConfig.getSparesByMonthYear
      ...
      alert("❌ Error fetching Spares: " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(sparesConfig.upload, file);
      alert("✅ Spares file uploaded successfully!");
      setFile(null);
    } catch (err) {
  ...
  // Generate columns dynamically (hide SINo)
  const columns = tableData[0]
    ? Object.keys(tableData[0])
        .filter((key) => key !== "sparesSINo")
        .map((key) => ({
          field: key,
          flex: 1,
        }))
  ...
  return (
    <Box sx={{ width: "100%" }}>
      {/* ✅ Reusable Navbar (only for navigation) */}
      <UploadNavbar
        buttons={[
...
export default SparesUploadPage;
