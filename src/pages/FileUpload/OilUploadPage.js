...
function OilUploadPage() {
  const oilConfig = apiModules.find((m) => m.name === "Oil");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = oilConfig.get;
      if (withFilter && month && year) {
        path = oilConfig.getOilByMonthYear
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(oilConfig.upload, file);
      alert("✅ Oil file uploaded successfully!");
      setFile(null);
    } catch (err) {
