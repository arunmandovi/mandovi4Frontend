...
function ReferenceeUploadPage() {
  const referenceeConfig = apiModules.find((m) => m.name === "Referencee");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = referenceeConfig.get;
      if (withFilter && month && year) {
        path = referenceeConfig.getReferenceeByMonthDate
      ...
      alert("❌ Error fetching Reference : " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(referenceeConfig.upload, file);
      alert("✅ Reference file uploaded successfully!");
      setFile(null);
    } catch (err) {
...
export default ReferenceeUploadPage;