...
function PMSPartsUploadPage() {
  const pmsPartsConfig = apiModules.find((m) => m.name === "PMS Parts");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = pmsPartsConfig.get;
      if (withFilter && selectedDate) {
        path = pmsPartsConfig.getPMSPartsByPMSDate.replace("{pmsDate}", selectedDate);
      }
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(pmsPartsConfig.upload, file);
...
export default PMSPartsUploadPage;