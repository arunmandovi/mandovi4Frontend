...
function PMSPartsUploadPage() {
  const config = apiModules.find((m) => m.name === "PMS Parts");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Are you sure you want to delete ALL PMS Parts data? This action cannot be undone.")) {
      return;
    }

    try {
      const res = await deleteData(config.deleteAll);
      alert("🗑️ All PMS Parts data deleted successfully!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="PMS Parts Upload File" />
