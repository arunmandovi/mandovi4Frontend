...
function ServiceeUploadPage() {
  const config = apiModules.find((m) => m.name === "Servicee");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Delete ALL Service data?")) return;

    setLoading(true);
    try {
      await deleteAllRecords(config.deleteAll);
      alert("🗑 All Service data deleted!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="Service Upload File" />
...
export default ServiceeUploadPage;
