...
function TATUploadPage() {
  const tatConfig = apiModules.find((m) => m.name === "TAT");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = tatConfig.get;
      if (withFilter && month && year) {
        path = tatConfig.getTATByMonthYear
      ...
      alert("❌ Error fetching TAT : " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(tatConfig.upload, file);
      alert("✅ TAT file uploaded successfully!");
      setFile(null);
    } catch (err) {
