...
function SAConversionUploadPage() {
  const config = apiModules.find((m) => m.name === "SA Conversion");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Delete ALL SA Conversion data?")) return;

    setLoading(true);
    try {
      await deleteAllRecords(config.deleteAll);
      alert("🗑 All SA Conversion data deleted!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="SA Conversion Upload File" />
...
export default SAConversionUploadPage;
