...
function MSGPProfitUploadPage() {
  const config = apiModules.find((m) => m.name === "MSGP Profit");
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm("⚠ Are you sure you want to delete ALL MSGP Profit data? This action cannot be undone.")) {
      return;
    }

    try {
      const res = await deleteData(config.deleteAll);
      alert("🗑️ All MSGP Profit data deleted successfully!");
      setTableData([]);
    } catch (err) {
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <Box sx={{ pt: "140px" }}>
        ...
        >
          <TitleBar title="MSGP Profit Upload File" />
