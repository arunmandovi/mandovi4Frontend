...
function LoaddUploadPage() {
  const loaddConfig = apiModules.find((m) => m.name === "Loadd");
  ...
  const handleFetch = async () => {
    try {
      const data = await fetchData(loaddConfig.get);
  ...
  const handleFilter = async () => {
    ...
    try {
      ...
      const url = `${loaddConfig.getLoaddByMonthYear}?${params.toString()}`;
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(loaddConfig.upload, file);
...
export default LoaddUploadPage;
