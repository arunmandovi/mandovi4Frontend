...
function LabourUploadPage() {
  const labourConfig = apiModules.find((m) => m.name === "Labour");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = labourConfig.get;
      if (withFilter && month && year) {
        path = labourConfig.getLabourByMonthYear
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(labourConfig.upload, file);
      alert("✅ Labour file uploaded successfully!");
      setFile(null);
    } catch (err) {
...
export default LabourUploadPage;