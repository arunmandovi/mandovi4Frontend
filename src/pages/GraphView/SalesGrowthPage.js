...
const SalesGrowthPage = () => {
  ...
  const chartData = useMemo(() => {
    ...
    sortedData.forEach((r) => {
      ...
      const prevCount = prevYearMap[r.branch];
      yearMap[r.year][r.branch] =
        prevCount !== undefined ? Number(((r.count - prevCount) / prevCount) * 100).toFixed(2) : 0;

      prevYearMap[r.branch] = r.count;
    });
      ...
      Object.keys(yearMap)
        ...
        .forEach((yr) => {
          const combined = BRANCHES.reduce((sum, br) => {
            const val = sortedData.find((d) => d.year === yr && d.branch === br)?.count || 0;
            return sum + val;
          }, 0);
          yearMap[yr]["ALL"] =
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h4">SALES GROWTH % (YEAR VS BRANCH)</Typography>
