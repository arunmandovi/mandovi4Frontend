    ...
    const fetchCitySummary = async () => {
      try {
        ...
        for (const m of activeMonths) {
          const res = await fetchData(`/api/mga_profit/mga_profit_summary?&months=${m}`);
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button
            variant="contained"
            color="secondary"
            onClick={() => navigate("/DashboardHome/mga_profit-bar-chart")}
          >
            CityWise
          </Button>
          ...
          <Button
            variant="contained"
            color="secondary"
            onClick={() => navigate("/DashboardHome/mga_profit_branches-bar-chart")}
          >
            BranchWise
          </Button>
        </Box>
