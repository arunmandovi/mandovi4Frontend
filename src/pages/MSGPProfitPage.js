...
function MSGPProfitPage() {
  const msgp_profitConfig = apiModules.find((m) => m.name === "MSGP Profit");
  ...
  const handleFetch = async (withFilter = false) => {
    try {
      let path = msgp_profitConfig.get;
      if (withFilter && month && year) {
        path = msgp_profitConfig.getMSGPProfitByMonthYear
      ...
      alert("❌ Error fetching Oil: " + err.message);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadFile(msgp_profitConfig.upload, file);
      alert("✅ MSGP Profit file uploaded successfully!");
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {/* Table */}
      <DataTable data={tableData} title="MSGP Profit" />
    </Box>
  );
}
