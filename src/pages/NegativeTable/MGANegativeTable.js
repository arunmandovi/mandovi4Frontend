...
const growthFormatConfig = { mgaVeh: { decimalPlaces: 0, showPercent: false },};
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/mga/mga_branch_summary?${params.toString()}`;
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h4">MGA Table (Branch-wise)</Typography>
      ...
      <NegativeTableView
        ...
        getNegativeCellSx={(value) => getThresholdCellSx(value, 475)}
      />
    </Box>
  );
}

export default MGANegativeTable;