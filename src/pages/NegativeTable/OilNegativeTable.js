...
const growthFormatConfig = {
  qtyFullSynthetic: percentFormat,
  qtySemiSynthetic: percentFormat,
  qtyFullSemiSynthetic: percentFormat,
};
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/oil/oil_branch_summary?${params.toString()}`;
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h4">OIL Table (Branch-wise)</Typography>
...
export default OilNegativeTable;