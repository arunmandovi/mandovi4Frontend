...
function SparesPage() {
  const [sparesSummary, setSparesSummary] = useState([]);
  ...
  const columnRenameMap = {
    ...
    previousBattery: "Battery 2024-25",
    currentBattery: "Battery 2025-260",
    growthBattery: "Battery Growth %",
    previousTyre: "Tyre 2024-25",
    currentTyre: "Tyre 2025-26",
    growthTyre: "Tyre Growth %",
  ...
  const combineDataSets = (dataSets, keyField) => {
    ...
    // ✅ Updated Grand Total Row Calculation for Last Row
const addGrandTotalRow = (data) => {
  ...
  allKeys.forEach((key) => {
    ...
    if (key.toLowerCase().includes("growth")) {
      ...
      if (key.includes("growthSRSpares")) {
      ...
      } else if (key.includes("growthBattery")) {
        const previousBattery = totalRow["previousBattery"] || 0;
        const currentBattery = totalRow["currentBattery"] || 0;
        sum = (currentBattery - previousBattery) * 100 / previousBattery;
      } else if (key.includes("growthTyre")) {
        const previousTyre = totalRow["previousTyre"] || 0;
        const currentTyre = totalRow["currentTyre"] || 0;
        sum = (currentTyre - previousTyre) * 100 / previousTyre;
      } else {
        sum = val;
      }
  ...
  useEffect(() => {
    const fetchSparesSummary = async () => {
      try {
        let responses = [];
  ...
  const renamedData = sparesSummary.map((row) => {
    const newRow = {};
  ...
  return (
    <Box className="battery-container" sx={{ p: 3 }}>
      ...
      <Box sx={{ overflowX: "auto", width: "100%" }}>
        <DataTable data={renamedData} title="Spares Summary" hiddenColumns={hiddenColumns} />
      </Box>
    </Box>
  );
}
