...
function SparesBarChartPage() {
  ...
  useEffect(() => {
    const prev = getSelectedGrowth("spares");

    const fromPages = location.state?.fromNavigation === true;

    if (!fromPages) {
      if (!prev) {
        setSelectedGrowthState("SR&BR Spares Growth %");
        setSelectedGrowth("SR&BR Spares Growth %", "spares");
      } else {
        setSelectedGrowthState(prev);
      }
    } else {
      setSelectedGrowthState(prev || "SR&BR Spares Growth %");
    }
  }, []);
