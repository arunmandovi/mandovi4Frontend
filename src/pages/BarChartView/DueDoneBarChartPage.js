  ...
  useEffect(() => {
    const fetchCitySummary = async () => {
      try {
        ...
        const data = await fetchData(`/api/due_done/due_done_summary${query}`);
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
        <Typography variant="h4">DUE VS DONE REPORT (City-wise)</Typography>
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="contained" color="secondary" onClick={() => navigate("/DashboardHome/due_done")}>Graph-CityWise</Button>
          <Button variant="contained" color="secondary" onClick={() => navigate("/DashboardHome/due_done")}>Graph-BranchWise</Button>
...
export default DueDoneBarChartPage;
