...
function RevenueBarChartPage() {
  ...
  useEffect(() => {
    const prev = getSelectedGrowth("revenue");

    const fromPages = location.state?.fromNavigation === true;

    if (!fromPages) {
      if (!prev) {
        setSelectedGrowthState("SR&Br Total Growth %");
        setSelectedGrowth("SR&Br Total Growth %", "revenue");
      } else {
        setSelectedGrowthState(prev);
      }
    } else {
      setSelectedGrowthState(prev || "SR&Br Total Growth %");
    }
  }, []);
...
export default RevenueBarChartPage;
