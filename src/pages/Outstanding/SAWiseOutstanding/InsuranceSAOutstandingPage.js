...
const InsuranceSAOutstandingPage = () => {
  ...
  useEffect(() => {
    ...
    if (advisorFilter.length > 0) {
      filtered = filtered.filter(row => {
        const advisorValue = row.salesMan || "null";
        return advisorFilter.includes(advisorValue);
      });
    }
  ...
  }, [workshopFilter, advisorFilter, data, sortConfig]);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        <Typography variant="h4" sx={{ fontWeight: 600 }}>
          Insurance Outstanding – Workshop Summary
        </Typography>
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="contained" onClick={() => navigate("/DashboardHome/insurance_branch_outstanding")}>BranchWise View</Button>
...
export default InsuranceSAOutstandingPage;
