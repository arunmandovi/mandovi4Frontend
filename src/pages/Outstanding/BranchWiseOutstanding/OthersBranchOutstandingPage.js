...
const OthersBranchOutstandingPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const res = await fetchData("/api/outstanding/others_branch_outstanding");
  ...
  useEffect(() => {
    if (selectedBranch && !showPartyView) {
      const loadSaData = async () => {
        try {
          setSaLoading(true);
          const res = await fetchData("/api/outstanding/others_sa_outstanding");
          ...
          const allAdvisors = branchData.reduce((acc, row) => {
            ...
            if (advisorValue && !acc.find(a => a.value === advisorValue)) {
              acc.push({
                label: advisorValue === "null" ? "" : advisorValue, // CHANGED: Empty label for null
                value: advisorValue
              });
            }
            return acc;
    ...
    if (selectedBranch && showPartyView) {
      const loadPartyData = async () => {
        try {
          ...
          const res = await fetchData("/api/outstanding/others_party_outstanding");
          
          // Always filter by branch first, then optionally by advisor
          let advisorData = res.filter(row => 
            (row.branchName === selectedBranch || row.segment === selectedBranch)
          );
          
          // If specific advisor selected, filter by advisor too
          if (selectedAdvisor && selectedAdvisor !== "null") {
            advisorData = advisorData.filter(row => row.salesMan === selectedAdvisor);
          }
  ...
  }, [selectedBranch, selectedAdvisor, showPartyView]);
  ...
  // Handle advisor click - works for null/empty values too
  const handleAdvisorClick = useCallback((advisorName) => {
    setSelectedAdvisor(advisorName || "null");
      ...
      {showPartyView ? (
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              ...
              <Typography variant="h4" sx={{ fontWeight: 600 }}>
                {selectedAdvisor && selectedAdvisor !== "null" ? selectedAdvisor : ""} - Party Details ({selectedBranch})
              </Typography>
            </Box>
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button variant="contained" onClick={() => navigate("/DashboardHome/others_party_outstanding")}>All Parties</Button>
            </Box>
          </Box>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {partyFilteredData.map((row, index) => (
                    <TableRow key={row.partyName || row.customerName || index} sx={{ 
        ...
        <>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button variant="contained" onClick={() => navigate("/DashboardHome/others_sa_outstanding")}>All SA</Button>
            </Box>
          </Box>
              ...
              <Table size="small">
                ...
                <TableBody>
                  {saFilteredData.map((row, index) => (
                    <TableRow 
                      key={`${row.salesMan || 'null'}-${row.segment || 'null'}-${index}`} 
                      sx={{ 
                        background: index % 2 ? "#fafafa" : "#fff",
                        cursor: 'pointer',
                        '&:hover': { backgroundColor: '#e3f2fd !important' }
                    ...
                    >
                      ...
                      {/* CHANGED: Show blank cell for empty advisor */}
        ...
        <>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            <Typography variant="h4" sx={{ fontWeight: 600 }}>
              Others Outstanding – Branch Summary
            </Typography>
            <Box sx={{ display: "flex", gap: 1 }}>
            ...
            <Table size="small">
              ...
              <TableBody>
                {filteredData.map((row, index) => (
                  <TableRow 
                    key={row.segment || row.branchName || index} 
                    sx={{ 
                      background: index % 2 ? "#fafafa" : "#fff",
                      cursor: 'pointer',
                      '&:hover': { backgroundColor: '#e3f2fd !important' }
...
export default OthersBranchOutstandingPage;
