...
const InvoiceBranchOutstandingPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        const res = await fetchData("/api/outstanding/invoice_branch_outstanding");
  ...
  useEffect(() => {
    if (selectedBranch && !showPartyView) {
      const loadSaData = async () => {
        try {
          setSaLoading(true);
          const res = await fetchData("/api/outstanding/invoice_sa_outstanding");
    ...
    if (selectedAdvisor && showPartyView) {
      const loadPartyData = async () => {
        try {
          ...
          const res = await fetchData("/api/outstanding/invoice_party_outstanding");
      ...
      {showPartyView ? (
        ...
        <>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button variant="contained" onClick={() => navigate("/DashboardHome/invoice_party_outstanding")}>All Parties</Button>
            </Box>
          </Box>
        ...
        <>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            ...
            <Box sx={{ display: "flex", gap: 1 }}>
              <Button variant="contained" onClick={() => navigate("/DashboardHome/invoice_sa_outstanding")}>All SA</Button>
            </Box>
          </Box>
        ...
        <>
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
            <Typography variant="h4" sx={{ fontWeight: 600 }}>
              Invoice Outstanding – Branch Summary
            </Typography>
            <Box sx={{ display: "flex", gap: 1 }}>
...
export default InvoiceBranchOutstandingPage;
