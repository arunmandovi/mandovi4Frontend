...
const CashPartyOutstandingPage = () => {
  ...
  const fetchDataWithFilters = async () => {
    try {
      ...
      const res = await fetchData(`/api/outstanding/cash_party_outstanding?${params.toString()}`);
  ...
  useEffect(() => {
    fetchDataWithFilters();
  }, [workshopFilter, advisorFilter, searchParty]);
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        <Typography variant="h4" sx={{ fontWeight: 600 }}>
          Cash Outstanding – Party Summary
        </Typography>
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="contained" onClick={() => navigate("/DashboardHome/cash_branch_outstanding")}>BranchWise View</Button>  
      ...
      {/* FILTERS - BULLETPROOF MULTI-SELECT */}
        ...
        <Table size="small">
          <TableHead>
            <TableRow sx={{ background: "#455a64" }}>
              ...
              <TableCell align="right" sx={{ color: "#fff", fontWeight: 800 }}>
                <TableSortLabel active={sortConfig.key === 'balanceAmt'} direction={sortConfig.direction} onClick={() => handleSort('balanceAmt')} sx={sortLabelSx}>
                  Balance Amt
                </TableSortLabel>
              </TableCell>
              <TableCell align="right" sx={{ color: "#fff", fontWeight: 800 }}>
                <TableSortLabel active={sortConfig.key === 'upToSeven'} direction={sortConfig.direction} onClick={() => handleSort('upToSeven')} sx={sortLabelSx}>
                  Upto 7 Days
                </TableSortLabel>
              </TableCell>
              <TableCell align="right" sx={{ color: "#fff", fontWeight: 800 }}>
                <TableSortLabel active={sortConfig.key === 'eightToThirty'} direction={sortConfig.direction} onClick={() => handleSort('eightToThirty')} sx={sortLabelSx}>
                  8-30 Days
                </TableSortLabel>
              </TableCell>
              <TableCell align="right" sx={{ color: "#fff", fontWeight: 800 }}>
                <TableSortLabel active={sortConfig.key === 'thirtyOneToNinty'} direction={sortConfig.direction} onClick={() => handleSort('thirtyOneToNinty')} sx={sortLabelSx}>
                  31-90 Days
                </TableSortLabel>
              </TableCell>
              <TableCell align="right" sx={{ color: "#fff", fontWeight: 800 }}>
                <TableSortLabel active={sortConfig.key === 'grtNinty'} direction={sortConfig.direction} onClick={() => handleSort('grtNinty')} sx={sortLabelSx}>
                  >90 Days
                </TableSortLabel>
              </TableCell>
            </TableRow>
          ...
          <TableBody>
            {filteredData.map((row, index) => (
              <TableRow key={`${(row.segment || "null")}-${(row.salesMan || "null")}-${row.partyName || 'unknown'}-${index}`} sx={{ background: index % 2 ? "#fafafa" : "#fff" }}>
            ...
            <TableRow sx={{ background: "#e3f2fd" }}>
              ...
              <TableCell align="right" sx={{ fontWeight: 900, fontSize: '1.1em', color: '#1976d2' }}>{formatCurrency(totals.balanceAmt)}</TableCell>
              <TableCell align="right" sx={{ fontWeight: 900, fontSize: '1.1em', color: '#1976d2' }}>{formatCurrency(totals.upToSeven)}</TableCell>
              <TableCell align="right" sx={{ fontWeight: 900, fontSize: '1.1em', color: '#1976d2' }}>{formatCurrency(totals.eightToThirty)}</TableCell>
              <TableCell align="right" sx={{ fontWeight: 900, fontSize: '1.1em', color: '#1976d2' }}>{formatCurrency(totals.thirtyOneToNinty)}</TableCell>
              <TableCell align="right" sx={{ fontWeight: 900, fontSize: '1.1em', color: '#1976d2' }}>{formatCurrency(totals.grtNinty)}</TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </TableContainer>
...
export default CashPartyOutstandingPage;
