...
function BatteryTyreBranchesBarChartPage() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/battery_tyre/battery_tyre_branch_summary${params.toString()? "?"+params.toString():""}`;
        const data = await fetchData(endpoint);
        setSummary(Array.isArray(data)?data:[]);
      } catch (error) {
        console.error("Error fetching battery tyre branch summary:", error);
        setSummary([]);
      }
    };
    fetchSummary();
  ...
  return (
    <Box sx={{p:3}}>
      ...
      {!selectedGrowth ? <Typography sx={{mt:2}}>👆 Select a growth type to view the chart below</Typography> :
        <BranchBarChart
          chartData={chartData}
          chartType={qtyProfitOptions.includes(selectedGrowth) ? "MGABranchesBarChart" : undefined}
          selectedGrowth={selectedGrowth}
          decimalPlaces={
...
export default BatteryTyreBranchesBarChartPage;
