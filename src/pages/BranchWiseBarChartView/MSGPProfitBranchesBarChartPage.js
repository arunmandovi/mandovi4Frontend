  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/msgp_profit/msgp_profit_branch_summary${
          params.toString() ? "?" + params.toString() : ""
        }`;
        const data = await fetchData(endpoint);
        setSummary(Array.isArray(data) ? data : []);
      } catch (error) {
        console.error("Error fetching msgp profit tyre branch summary:", error);
        setSummary([]);
      }
    };
    fetchSummary();
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="contained" color="secondary" onClick={() => navigate("/DashboardHome/msgp_profit")}>
            Graph
          </Button>
          <Button variant="contained" color="secondary" onClick={() => navigate("/DashboardHome/msgp_profit-bar-chart")}>
            CityWise
          </Button>
        </Box>
      </Box>
