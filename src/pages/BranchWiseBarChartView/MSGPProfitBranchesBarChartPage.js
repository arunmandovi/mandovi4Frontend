  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/msgp_profit/msgp_profit_branch_summary${params.toString()? "?"+params.toString():""}`;
        const data = await fetchData(endpoint);
        setSummary(Array.isArray(data)?data:[]);
      } catch (error) {
        console.error("Error fetching msgp profit branch summary:", error);
        setSummary([]);
      }
    };
    fetchSummary();
