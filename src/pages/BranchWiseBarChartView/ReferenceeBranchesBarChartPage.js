...
function ReferenceeBranchesBarChartPage() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/referencee/referencee_branch_summary${
          params.toString() ? "?" + params.toString() : ""
        }`;
        const data = await fetchData(endpoint);
        setSummary(Array.isArray(data) ? data : []);
      } catch (error) {
        console.error("Error fetching reference tyre branch summary:", error);
        setSummary([]);
      }
    };
    fetchSummary();
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      >
        <Typography variant="h4">REFERENCE REPORT (Branch-wise)</Typography>
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="contained" color="secondary" onClick={() => navigate("/DashboardHome/referencee")}>
            Graph
          </Button>
...
export default ReferenceeBranchesBarChartPage;
