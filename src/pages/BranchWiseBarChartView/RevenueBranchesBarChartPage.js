...
function RevenueBranchesBarChartPage() {
  ...
  useEffect(() => {
    const fetchSummary = async () => {
      try {
        ...
        const endpoint = `/api/revenue/revenue_branch_summary${
          params.toString() ? "?" + params.toString() : ""
        }`;
        const data = await fetchData(endpoint);
        setSummary(Array.isArray(data) ? data : []);
      } catch (error) {
        console.error("Error fetching revenue tyre branch summary:", error);
        setSummary([]);
      }
    };
    fetchSummary();
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        ...
        <Box sx={{ display: "flex", gap: 1 }}>
          <Button variant="contained" color="secondary" onClick={() => navigate("/DashboardHome/revenue")}>
            Graph
          </Button>
...
export default RevenueBranchesBarChartPage;
